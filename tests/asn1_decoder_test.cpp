...
TEST_F(Asn1DecoderTest, Empty_Failure) {
    ...
    EXPECT_EQ(0, asn1_constructed_skip_all(ctx));
    EXPECT_EQ(0, asn1_constructed_type(ctx));
    EXPECT_EQ(NULL, asn1_sequence_get(ctx));
    EXPECT_EQ(NULL, asn1_set_get(ctx));
    EXPECT_EQ(0, asn1_sequence_next(ctx));

    uint8_t* junk;
    size_t length;
    EXPECT_EQ(0, asn1_oid_get(ctx, &junk, &length));
    EXPECT_EQ(0, asn1_octet_string_get(ctx, &junk, &length));
...
TEST_F(Asn1DecoderTest, ConstructedGet_TooSmallForChild_Failure) {
    ...
    uint8_t* oid;
    size_t length;
    EXPECT_EQ(0, asn1_oid_get(ptr, &oid, &length));
...
TEST_F(Asn1DecoderTest, ConstructedGet_Success) {
    ...
    uint8_t* oid;
    size_t length;
    ASSERT_EQ(1, asn1_oid_get(ptr, &oid, &length));
    EXPECT_EQ(1U, length);
    EXPECT_EQ(0x01U, *oid);
    asn1_context_free(ptr);
    asn1_context_free(ctx);
...
TEST_F(Asn1DecoderTest, ConstructedSkipAll_Success) {
    uint8_t data[] = { 0xA0, 0x03, 0x02, 0x01, 0x01,
                            ...
                            0x06, 0x01, 0xA5, };
    asn1_context_t* ctx = asn1_context_new(data, sizeof(data));
    EXPECT_EQ(1, asn1_constructed_skip_all(ctx));
    uint8_t* oid;
    size_t length;
    ASSERT_EQ(1, asn1_oid_get(ctx, &oid, &length));
    EXPECT_EQ(1U, length);
    EXPECT_EQ(0xA5U, *oid);
    asn1_context_free(ctx);
}
...
TEST_F(Asn1DecoderTest, OidGet_LengthZero_Failure) {
    ...
    uint8_t* oid;
    size_t length;
    ASSERT_EQ(0, asn1_oid_get(ctx, &oid, &length));
    asn1_context_free(ctx);
}
...
TEST_F(Asn1DecoderTest, OctetStringGet_LengthZero_Failure) {
    ...
    uint8_t* string;
    size_t length;
    ASSERT_EQ(0, asn1_octet_string_get(ctx, &string, &length));
    asn1_context_free(ctx);
}
...
TEST_F(Asn1DecoderTest, OctetStringGet_Success) {
    ...
    uint8_t* string;
    size_t length;
    ASSERT_EQ(1, asn1_octet_string_get(ctx, &string, &length));
    EXPECT_EQ(1U, length);
